    ...
    internal class Banco_Caixa : AbstractBanco, IBanco
    {
        ...
        public override void FormataCodigoBarra(Boleto boleto)
        {
            ...
            //{
                ...
                //Posição 35 - 43
                string terceiraParteNossoNumero = boleto.NossoNumero.Substring(8);
        ...
        public override void FormataLinhaDigitavel(Boleto boleto)
        {
            ...
            ///   Data..: 01/03/2011
            /// </summary>
           // if (boleto.NossoNumero.Length == 17 || boleto.NossoNumero.Length == 10)
           // {
                #region Campo 1
               ...
               // Grupo4 = Strings.Mid(boleto.CodigoBarra.Codigo, 5, 1);
        ...
        public override void FormataNossoNumero(Boleto boleto)
        {
           ...
           // boleto.NossoNumero = boleto.NossoNumero.Substring(0, 16);
        ...
        {
            return GerarDetalheSegmentoPRemessaCNAB240(boleto, numeroRegistro, TipoArquivo.CNAB240);
        }
        ...
        public string GerarHeaderRemessaCNAB240(Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                header += Utils.FitStringLength(cedente.CPFCNPJ, 14, 14, '0', 0, true, true, true);     // CPF/CNPJ do cedente
                header += Utils.FormatCode("0", "0", 20);                                               // Código do Convênio no Banco 

                header += Utils.FitStringLength(cedente.ContaBancaria.Agencia, 5, 5, '0', 0, true, true, true);     // CPF/CNPJ do cedente

                header += Utils.FormatCode(cedente.ContaBancaria.DigitoAgencia, "0", 1);                // Dígito Verificador da Agência 
                header += Utils.FormatCode(cedente.Codigo.ToString(), "0", 6, true );                       // Código do Cedente (sem operação)  
                header += Utils.FormatCode("0", "0", 8);
                
                header += Utils.FormatCode(cedente.Nome, " ", 30, false );                                      // Nome do cedente
                header += Utils.FormatCode("CAIXA ECONOMICA FEDERAL", " ", 30, false );                         // Nome do Banco
                ...
                //header += "000001";     
                header += numeroArquivoRemessa.ToString("000000");                                    // Número Seqüencial do Arquivo 
                header += "050";                                                                        // Número da Versão do Layout do Arquivo 
                header += "00000";                                                                          // Densidade de Gravação do Arquivo 
        ...
        private string GerarHeaderLoteRemessaCNAB240(Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                header += (cedente.CPFCNPJ.Length == 11 ? "1" : "2");                                   // Tipo de Inscrição 
                header += Utils.FitStringLength(cedente.CPFCNPJ, 15, 15, '0', 0, true, true, true);     // CPF/CNPJ do cedente
                header += Utils.FormatCode(cedente.Codigo.ToString(), "0", 6, true );                          // Código do Convênio no Banco 
                header += Utils.FormatCode("", "0", 14);                                                 // Uso Exclusivo CAIXA
                header += Utils.FitStringLength(cedente.ContaBancaria.Agencia, 5, 5, '0', 0, true, true, true);     // CPF/CNPJ do cedente 
                header += Utils.FormatCode(cedente.ContaBancaria.DigitoAgencia, "0", 1);                // Dígito Verificador da Agência 
                header += cedente.Codigo;                                                              // cÓDIGO 6 POS
        ...
        public string GerarDetalheSegmentoPRemessaCNAB240(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                iQtd = iQtd  + 1;
                dValor = dValor + boleto.ValorBoleto;


                string header = Utils.FormatCode(Codigo.ToString(), "0", 3, true);                      // código do banco na compensação
                header += "0001";                                                                       // Lote de Serviço
                header += "3";                                                                          // Tipo de Registro 
                header += Utils.FormatCode(numeroRegistro.ToString(), "0", 5, true);                          // Nº Sequencial do Registro no Lote 
                header += "P";                                                                          // Cód. Segmento do Registro Detalhe
                header += " ";                                                                          // Uso Exclusivo FEBRABAN/CNAB
                header += "01";                                                                         // Código de Movimento Remessa 
                header += Utils.FitStringLength(boleto.Cedente.ContaBancaria.Agencia, 5, 5, '0', 0, true, true, true);     // CPF/CNPJ do cedente
                header += Utils.FormatCode(boleto.Cedente.ContaBancaria.DigitoAgencia, "0", 1);                                          // Dígito Verificador da Agência 
                header += boleto.Cedente.Codigo;     // CPF/CNPJ do cedente
                header += Utils.FormatCode("", "0", 11);


                //header += "14";
                //header += Utils.FormatCode(boleto.NossoNumero, "0", 15, true );                                // Identificação do Título no Banco 

                header += Utils.FormatCode(boleto.NossoNumero, "0", 17, true);                                // Identificação do Título no Banco 
                
                    
                
                header += "1";                                                                         // Código da Carteira 
                header += "1";                                        // Forma de Cadastr. do Título no Banco  -- registro
                // '1' = Com Cadastramento (Cobrança Registrada) 
                // '2' = Sem Cadastramento (Cobrança sem Registro) 
                header += "2";                                                                          // Tipo de Documento 
                header += "2";                                                                           // Identificação da Emissão do Bloqueto - cliente
                header += "0";                                                                          // Identificação da Distribuição - cliente
                header += Utils.FormatCode(boleto.NumeroDocumento, " ", 11, false);                            // Número do Documento de Cobrança 
                header += "    ";                                                                       // Uso Exclusivo CAIXA
                header += boleto.DataVencimento.ToString("ddMMyyyy");                                   // Data de Vencimento do Título
                header += Utils.FormatCode(boleto.ValorBoleto.ToString().Replace(",", "").Replace(".", ""), "0", 15, true); // Valor Nominal do Título 13
               
                //header += Utils.FormatCode(boleto.Cedente.ContaBancaria.Agencia, "0", 5, true);                      // Agência Encarregada da Cobrança 
                //header += Utils.FormatCode(boleto.Cedente.ContaBancaria.DigitoAgencia, "0", 1);                                         // Dígito Verificador da Agência 
                header += Utils.FormatCode("", "0", 6);
                
                header += Utils.FormatCode(boleto.EspecieDocumento.Codigo.ToString(), "0", 2, true);                               // Espécie do Título 
                ...
                header += Utils.FormatCode(boleto.ValorMulta.ToString().Replace(",", "").Replace(".", ""), "0", 15,true ); // Juros de Mora por Dia/Taxa 
                ...
                header += Utils.FormatCode(boleto.ValorDesconto.ToString().Replace(",", "").Replace(".", ""), "0", 15, true ); // Valor/Percentual a ser Concedido 
                header += Utils.FormatCode(boleto.IOF.ToString().Replace(",", "").Replace(".", ""), "0", 15, true ); // Valor do IOF a ser Recolhido 
                header += Utils.FormatCode(boleto.Abatimento.ToString().Replace(",", "").Replace(".", ""), "0", 15, true ); // Valor do Abatimento 
                header += Utils.FormatCode(boleto.NumeroDocumento, " ", 25, false);                                               // Identificação do Título na Empresa
                ...
                // header += diasDevolucao.ToString("000");                                                 // Número de Dias para Baixa/Devolução 3 posi
                ...
                header += Utils.FormatCode("", "0", 10);                                                // Uso Exclusivo FEBRABAN/CNAB 
        ...
        public string GerarDetalheSegmentoQRemessaCNAB240(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                header += Utils.FitStringLength(boleto.Sacado.CPFCNPJ, 15, 15, '0', 0, true, true, true);     // CPF/CNPJ do cedente

                header += Utils.FormatCode(boleto.Sacado.Nome, " ", 40, false );                                // Nome
                header += Utils.FormatCode(boleto.Sacado.Endereco.End, " ", 40,false );                        // Endereço
                header += Utils.FormatCode(boleto.Sacado.Endereco.Bairro, " ", 15, false);                     // Bairro 
                header += boleto.Sacado.Endereco.CEP;                                                   // CEP + Sufixo do CEP
                header += Utils.FormatCode(boleto.Sacado.Endereco.Cidade, " ", 15, true);                     // Cidade 
        ...
        public string GerarDetalheSegmentoRRemessaCNAB240(Boleto boleto, int numeroRegistroDetalhe, TipoArquivo CNAB240)
        {
            try
            {
                ...
                header += Utils.FormatCode(boleto.ValorMulta.ToString().Replace(",", "").Replace(".", ""), "0", 13, true ); // Valor/Percentual a Ser Aplicado
        ...
        public string GerarTrailerLoteRemessaCNAB240(int numeroRegistro)
        {
            try
            {
                ...
                header += Utils.FormatCode(numeroRegistro.ToString(), "0", 6, true );                          // Nº Sequencial do Registro no Lote     
                header += Utils.FormatCode(iQtd.ToString(), "0", 6,true);                                                 // Quantidade de Títulos em Cobrança
                header += Utils.FormatCode(dValor.ToString().Replace(",", ""), "0", 17, true);                                                // Valor Total dos Títulos em cobrança

                header += Utils.FormatCode(iQtd.ToString(), "0", 6,true);                                                 // Quantidade de Títulos em carteiras
                header += Utils.FormatCode(dValor.ToString().Replace(",",""), "0", 17,true);                                                // Valor Total dos Títulos em carteieras

                header += Utils.FormatCode(iQtd.ToString(), "0", 6,true);                                                 // Quantidade de Títulos em carteiras
                header += Utils.FormatCode(iQtd.ToString(), "0", 17,true);                                                 // Quantidade de Títulos em carteiras
