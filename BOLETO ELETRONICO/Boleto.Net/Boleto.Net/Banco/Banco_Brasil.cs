...
namespace BoletoNet
{
    ...
    internal class Banco_Brasil : AbstractBanco, IBanco
    {
        ...
        public override void ValidaBoleto(Boleto boleto)
        {
            ...
            if (boleto.Carteira.Equals("17"))
            {
                switch (boleto.Cedente.Convenio.ToString().Length)
                {
                    ...
                    case 7:
                        ...
                        boleto.NossoNumero = string.Format("{0}{1}", boleto.Cedente.Convenio, Utils.FormatCode(boleto.NumeroDocumento, 10));
                        break;
                    default:
            ...
            if (boleto.Cedente.ContaBancaria.Agencia.Length > 5)
                throw new NotImplementedException("A quantidade de dígitos da Agência " + boleto.Cedente.ContaBancaria.Agencia + ", são de 5 números.");
        ...
        public override void FormataCodigoBarra(Boleto boleto)
        {
            ...
            #endregion Carteira 18-140
            _dacBoleto = Mod11(boleto.CodigoBarra.Codigo.Replace("-",""), 9);
        ...
        public override void FormataLinhaDigitavel(Boleto boleto)
        {
            ...
            V = Valor do Título (*)
             */
            icampo5 = Convert.ToInt64(Strings.Mid(boleto.CodigoBarra.Codigo.Replace("-",""), 6, 14));
        ...
        public string GerarHeaderRemessaCNAB240(Cedente cedente)
        {
            try
            {
                ...
                if (cedente.Carteira.Length == 2)
                    if (cedente.Convenio == 2824961)
                        _header += cedente.Carteira.ToString() + "027  ";
                    else
                ...
                //if (cedente.Convenio == 2824961)
                //    _header += "002824961001417027  ";
                //else
                //{
                //    if (cedente.Convenio == 2824961)
                //        _header += "002824961001417019  ";
                //    else
                //        _header += cedente.Carteira.ToString() + "019  ";
                //}
                _header += Utils.FitStringLength(cedente.ContaBancaria.Agencia, 5, 5, '0', 0, true, true, true);
                if (cedente.ContaBancaria.Agencia == "1213")
                    _header += "0";
                else
        ...
        private string GerarHeaderLoteRemessaCNAB240(string numeroConvenio, Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                _headerLote += Utils.FitStringLength(cedente.Convenio.ToString(), 9, 9, '0', 0, true, true, true);
                _headerLote += "0014";
                // O Código da carteira é dividida em 2 partes:
                // - nº da carteira 9(02)
                // - variação (se houver) 9(03)
                ...
                if (cedente.Carteira.Length == 2)
                    if (cedente.Convenio == 2824961)
                        _headerLote += cedente.Carteira.ToString() + "027  ";
                    else
        ...
        public override string GerarDetalheSegmentoPRemessa(Boleto boleto, int numeroRegistro, string numeroConvenio)
        {
            try
            {
                ...
                _segmentoP += "P " + boleto.TipoOcorrencia;
                ...
                switch (boleto.Cedente.Convenio.ToString().Length)
                {
                    ...
                    case 7:
                        ...
                        if (boleto.NossoNumero.Length != 17)
                            _nossoNumero = string.Format("{0}{1}", boleto.Cedente.Convenio, Utils.FormatCode(boleto.NumeroDocumento, 10));
                        else
