...
{
    /// <summary>
    /// Classe referente ao Banco do Sicredi
    /// </summary>
    internal class Banco_Sicredi : AbstractBanco, IBanco
    {
        ...
        private int _numeroArquivoRemessa = 0;

        private int _QtdBoleto = 0;
        ...
        /// <summary>
        /// Validações particulares do Banco Sicredi
        /// </summary>
        public override void ValidaBoleto(Boleto boleto)
        {
            ...
            boleto.NossoNumero = Utils.FormatCode(boleto.NossoNumero, 8) + DigNossoNumeroSicredi(boleto);
        ...
        public string DigNossoNumeroSicredi(Boleto boleto)
        {
            ...
            string posto = "71"; //código do posto beneficiário (pp)
            string cedente = boleto.Cedente.ContaBancaria.Conta + boleto.Cedente.ContaBancaria.DigitoConta;                   //código do beneficiário (ccccc)
        ...
        public override void FormataCodigoBarra(Boleto boleto)
        {
            ...
            string sCampoLivre = "1" + "1"; //Carteira com registro e simples

            //71 = Posto
            sCampoLivre += Utils.FormatCode(boleto.NossoNumero, 9) + boleto.Cedente.ContaBancaria.Agencia + "71" + boleto.Cedente.ContaBancaria.Conta + "10";
            sCampoLivre += digSicredi(sCampoLivre);

            //string sCodigoBarras = "7489X";

            //string sCodigoBarras = string.Format("{0}{1}{2}{3}{4}{5}", Utils.FormatCode(Codigo.ToString(), 3), boleto.Moeda, FatorVencimento(boleto), valorBoleto, cmp_livre, digSicredi(cmp_livre).ToString());

            string sCodigoBarras = "7489";
            sCodigoBarras += FatorVencimento(boleto).ToString();
            sCodigoBarras += Utils.FormatCode(boleto.ValorBoleto.ToString("f").Replace(",", "").Replace(".", ""), 10);
            sCodigoBarras += sCampoLivre;

            int _dacBoleto = digSicredi(sCodigoBarras);

            if (_dacBoleto == 0 || _dacBoleto > 9)
                _dacBoleto = 1;

            boleto.CodigoBarra.Codigo = Strings.Left(sCodigoBarras, 4) + _dacBoleto + Strings.Right(sCodigoBarras, 39);


            ////Concatena o vencimento
            //sCodigoBarras += FatorVencimento(boleto).ToString();


            ////Concatena o valor do boleto
            //sCodigoBarras += Utils.FormatCode(boleto.ValorBoleto.ToString("f").Replace(",", "").Replace(".", ""), 10);


            //sCodigoBarras += sCampoLivre;

            //boleto.CodigoBarra.Codigo = sCodigoBarras; 
        ...
        public string GerarHeaderRemessaCNAB240(Cedente cedente)
        {
            try
            {
                ...
                if (cedente.ContaBancaria.DigitoConta != "")
                {
                    _header += Utils.FitStringLength(cedente.ContaBancaria.Conta + cedente.ContaBancaria.DigitoConta, 12, 12, '0', 0, true, true, true);
                    _header += Utils.FitStringLength(cedente.ContaBancaria.DigitoConta, 1, 1, ' ', 0, true, true, false);
                }
                else
                {
                    string sConta = cedente.ContaBancaria.Conta;
                    string sDigito = sConta.Substring(sConta.Length - 1, 1);


                    _header += Utils.FitStringLength(sConta, 12, 12, '0', 0, true, true, true);
                    _header += Utils.FitStringLength(sDigito, 1, 1, ' ', 0, true, true, false);
                }


                _header += " "; // NÃO UTILIZADO


                _header += Utils.FitStringLength(cedente.Nome, 30, 30, ' ', 0, true, true, false);
                _header += Utils.FitStringLength("SICREDI", 30, 30, ' ', 0, true, true, false);
                _header += _brancos10;
                _header += "1";
                ...
                //_header += "000001";
                _header += _numeroArquivoRemessa.ToString().PadLeft(6, '0');
        ...
        public string GerarHeaderRemessaCNAB400(Cedente cedente)
        {
            try
            {
                ...
                _header += Utils.FitStringLength(cedente.Nome, 30, 30, ' ', 0, true, true, false);
                _header += Utils.FitStringLength("BANCO SICREDI", 30, 30, ' ', 0, true, true, false);
                _header += _brancos10;
                _header += "1";
                _header += DateTime.Now.ToString("ddMMyyyy");
                _header += DateTime.Now.ToString("hhMMss");
        ...
        private string GerarHeaderLoteRemessaCNAB240(string numeroConvenio, Cedente cedente, int numeroArquivoRemessa)
        {
            try
            {
                ...
                _headerLote += _brancos20;


                _headerLote += Utils.FitStringLength(cedente.ContaBancaria.Agencia, 5, 5, '0', 0, true, true, true);
                _headerLote += Utils.FitStringLength(cedente.ContaBancaria.DigitoAgencia, 1, 1, ' ', 0, true, true, true);
                ...
                else
                {
                    ...
                    _headerLote += Utils.FitStringLength(sConta, 12, 12, '0', 0, true, true, true);
                    _headerLote += Utils.FitStringLength(sDigito, 1, 1, ' ', 0, true, true, false);
                }
        ...
        public override string GerarDetalheSegmentoPRemessa(Boleto boleto, int numeroRegistro, string numeroConvenio)
        {
            ...
            {
                _QtdBoleto += 1;
                _ValorBoleto += boleto.ValorBoleto;
                ...
                _segmentoP += Utils.FitStringLength(boleto.Cedente.ContaBancaria.DigitoAgencia, 1, 1, ' ', 0, true, true, true);
                if (boleto.Cedente.ContaBancaria.DigitoConta != "")
                {
                    _segmentoP += Utils.FitStringLength(boleto.Cedente.ContaBancaria.Conta + boleto.Cedente.ContaBancaria.DigitoConta, 12, 12, '0', 0, true, true, true);
                    _segmentoP += Utils.FitStringLength(boleto.Cedente.ContaBancaria.DigitoConta, 1, 1, ' ', 0, true, true, false);
                }
                else
                {
                    string sConta = boleto.Cedente.ContaBancaria.Conta;
                    string sDigito = sConta.Substring(sConta.Length - 1, 1);


                    _segmentoP += Utils.FitStringLength(sConta, 12, 12, '0', 0, true, true, true);
                    _segmentoP += Utils.FitStringLength(sDigito, 1, 1, ' ', 0, true, true, false);
                }
                ...
                _segmentoP += Utils.FitStringLength(boleto.NossoNumero, 20, 20, ' ', 0, true, true, false);

                // Informar 1 – modalidade Simples; 
                _segmentoP += "1";
                ...
                if (boleto.ValorDesconto > 0)
                {
                    ...
                    _segmentoP += "100000000000000000000000";
        ...
        public override string GerarDetalheSegmentoQRemessa(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            try
            {
                ...
                _segmentoQ += "0";
                _segmentoQ += "000000000000000"; //somente governo
        ...
        public override string GerarTrailerLoteRemessa(int numeroRegistro)
        {
            try
            {
                ...
                _trailerLote = "74800015         ";
                _trailerLote += Utils.FitStringLength(numeroRegistro.ToString(), 6, 6, '0', 0, true, true, true);

                _trailerLote += Utils.FitStringLength("0", 6, 6, '0', 0, true, true, true);
                _trailerLote += Utils.FitStringLength("0", 17, 17, '0', 0, true, true, true);
                _trailerLote += Utils.FitStringLength("0", 6, 6, '0', 0, true, true, true);
                _trailerLote += Utils.FitStringLength("0", 17, 17, '0', 0, true, true, true);
                _trailerLote += Utils.FitStringLength("0", 6, 6, '0', 0, true, true, true);

                //tOTAIS
                _trailerLote += Utils.FitStringLength(_ValorBoleto.ToString("0.00").Replace(",", ""), 17, 17, '0', 0, true, true, true);
                _trailerLote += Utils.FitStringLength(_QtdBoleto.ToString(), 6, 6, '0', 0, true, true, true);
                _trailerLote += Utils.FitStringLength(_ValorBoleto.ToString("0.00").Replace(",", ""), 17, 17, '0', 0, true, true, true);

                //_trailerLote += new string(' ', 17);
                //_trailerLote += Utils.FitStringLength(_QtdBoleto.ToString(), 6, 6, '0', 0, true, true, true);

                //_trailerLote += Utils.FitStringLength(_QtdBoleto.ToString(), 6, 6, '0', 0, true, true, true);

                _trailerLote += new string(' ', 125);
        ...
        public override string GerarTrailerArquivoRemessa(int numeroRegistro)
        {
            try
            {
                ...
                _trailerArquivo = "74899999         000001";
        ...
        internal static string Mod11BancoSicredi(string value)
        {
