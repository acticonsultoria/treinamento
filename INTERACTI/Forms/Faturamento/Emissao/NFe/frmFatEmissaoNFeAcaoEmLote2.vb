...
Public Class frmFatEmissaoNFeAcaoEmLote2
...
#Region "::: VARIAVEIS :::"
    ...
    Private Structure MensagemErroNovas
        Dim CodigoConsultora As String
        Dim Setor As String
        Dim RazaoSocial As String
    End Structure

    Private Structure MensagemErroImposto
...
#Region "::: NFE :::"
    ...
    Private Sub BoletoNFeMesmaPagina(ByVal sCodigoEmissaoCapa As String)
        Try
            ...
            If VerificaDireito(Formulario.FaturamentoEmissaoNFe, gcPrint) = True Then
                ...
                Dim sCodigoTituloCapa As String = oClsFrmFatEmissaoNFeTransmissaoLote.GerarBoleto(sCodigoEmissaoCapa)
                ...
                For i As Integer = 0 To oVetor.Count - 1
                    'Gera Arquivo
                    oClsFrmFinReceitaBoletoBancario.LoadArquivoBoletoLoteCrystalNFE(oVetor(i), sDiretorio, sArquivoBoleto, iCodigoBanco)
                ...
                goCrystalReport.sReport = goCrystalReport.sPath & "FAT000000003_BOLETO_NFE.rpt"

                Dim oReport As New usrReport
                oReport.Dock = DockStyle.Fill


                frmMain.LoadPageReport("FAT000000003_BOLETO_NFE", "DANFE - Documento Auxiliar de Nota Fiscal Eletrônica", oReport, frmMain.tabMain)
    ...
    Private Sub btnBoletoNFeMesmaPagina_Click(sender As Object, e As EventArgs) Handles btnBoletoNFeMesmaPagina.Click
        Try
            ...
            Else
                ...
                Call BoletoNFeMesmaPagina(sCodigoEmissaoCapa)
            End If
    ...
    Private Sub btnRelatorioNovas_Click(sender As Object, e As EventArgs) Handles btnRelatorioNovas.Click
        Try

            RelatorioNovas()
            ' ValidarImposto(False)
        Catch ex As Exception
    ...
    Private Sub btnEnviarSMS_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEnviarSMS.Click
        Try
            For i As Integer = 0 To grdNotaFiscalLote.RecordCount - 1
                ...
                If grdNotaFiscalLote.GetValue("telefone2") <> "" And IsDBNull(grdNotaFiscalLote.GetValue("telefone2")) = False Then
                    EnviarSMS_Telefone(grdNotaFiscalLote.GetValue("telefone2"))
                End If
                If grdNotaFiscalLote.GetValue("telefone_recado") <> "" And IsDBNull(grdNotaFiscalLote.GetValue("telefone_recado")) = False Then
...
#Region "::: FUNCTION / SUB :::"
            ...
            If VerificaDireito(Formulario.FaturamentoEmissaoNFe, gcPrint) = True Then
                ...
                For iRow As Integer = 0 To grdNotaFiscalLote.RecordCount - 1
                    ...
                    If grdNotaFiscalLote.GetValue("email_destinatario") <> "" Then
                        ...
                        Dim bBoleto As Boolean = chkBoleto.Checked
                        Dim bNFe As Boolean = chkNFe.Checked
                        Dim bSMS As Boolean = chkSMS.Checked

                        ' grdNotaFiscalLote.GetValue("email_destinatario")
                        'Rotina para enviar o email

                        Call EmailNFe(grdNotaFiscalLote.GetValue("codigo"), grdNotaFiscalLote.GetValue("email_destinatario"), grdNotaFiscalLote.GetValue("status"), bEnvioDireto, _
                                      bBoleto, bNFe, bSMS)
                    ...
                    If chkSMS.Checked = True Then
                        Try
                            ...
                            EnviarSMS_Telefone(grdNotaFiscalLote.GetValue("telefone_recado"))
                        Catch ex As Exception
                            grdNotaFiscalLote.SetValue("mensagem", "Não foi possivel enviar a SMS.")
                        End Try
                    End If
    ...
    Private Sub ValidarImposto(ByVal bTransmissao As Boolean, _
                             Optional ByRef bValidacao As Boolean = False)
        Try


            'Seta o retorno
            Dim sRetorno As String = oClsFrmFatEmissaoNFeTransmissaoLote.ValidarImposto(btnValidarEstoque.Tag, bTransmissao)

            If sRetorno = "" Then
                bValidacao = True
                If bTransmissao = False Then
                    MsgBox("Produtos validados com sucesso!", MsgBoxStyle.Information, "Validando os impostos")
                End If
                Exit Sub
            End If
            ...
            'Seta a mnesagem de erro
            Dim oMensagemErroImposto(oVetorRetorno.Count - 1) As MensagemErroImposto
            ...
            For i As Integer = 0 To oVetorRetorno.Count - 1
                ...
                oMensagemErroImposto(i).NotaFiscal = oVetorMensagem(0)
                oMensagemErroImposto(i).NumeroItem = oVetorMensagem(1)
                oMensagemErroImposto(i).CodigoProduto = oVetorMensagem(2)
                oMensagemErroImposto(i).Descricao = oVetorMensagem(3)
                oMensagemErroImposto(i).Status = oVetorMensagem(4)

            Next

            'Seta o relatório de erros
            Call ReportErroImposto("Faturamento - Validação de Imposto", _
                                   "Validação de Imposto", _
                                   oMensagemErroImposto)
            bValidacao = False
    ...
    Private Sub RelatorioNovas()
        Try
            ...
            oClsFrmFatEmissaoNFeTransmissaoLote.RelatorioNovas(sCodigoEmissaoCapa)
            ...
            ''Seta a mnesagem de erro
            'Dim oMensagemErroNovas(oVetorRetorno.Count - 1) As MensagemErroNovas
            ...
            '    oMensagemErroNovas(i).CodigoConsultora = oVetorMensagem(0)
            '    oMensagemErroNovas(i).Setor = oVetorMensagem(1)
            '    oMensagemErroNovas(i).RazaoSocial = oVetorMensagem(2)

            'Next

            ''Seta o relatório de erros
            'Call ReportErroNovas("Faturamento - Relatório de Novas", _
            '                       "Relatório de Novas", _
            '                       oMensagemErroNovas)
    ...
    Private Function ReportErroImposto(ByVal sTitle As String, _
                               ByVal sSubTitle As String, _
                               ByVal oMensagem() As MensagemErroImposto) As String
        ...
        'create the header row
        sReportHTML &= "<tr>"
        sReportHTML &= "<td width=10% vAlign=top align=left bgcolor=#99CCFF><strong><font size=2 face=Arial, Helvetica, sans-serif>NOTA FISCAL</font></strong></td>"
        sReportHTML &= "<td width=10% vAlign=top align=left bgcolor=#99CCFF><strong><font size=2 face=Arial, Helvetica, sans-serif>NÚMERO ITEM</font></strong></td>"
        sReportHTML &= "<td width=20% vAlign=top align=left bgcolor=#99CCFF><strong><font size=2 face=Arial, Helvetica, sans-serif>CÓDIGO PRODUTO</font></strong></td>"
        sReportHTML &= "<td width=30% vAlign=top align=left bgcolor=#99CCFF><strong><font size=2 face=Arial, Helvetica, sans-serif>DESCRIÇÃO</font></strong></td>"
        sReportHTML &= "<td width=40% vAlign=top align=left bgcolor=#99CCFF><strong><font size=2 face=Arial, Helvetica, sans-serif>STATUS</font></strong></td>"
        sReportHTML &= "</tr>"
        ...
        For i As Integer = 0 To UBound(oMensagem)
            ...
            sReportHTML &= "<tr>"
            sReportHTML &= "<td vAlign=top align=left><font size=2 face=Arial, Helvetica, sans-serif>" & oMensagem(i).NotaFiscal & "</font></td>"
            sReportHTML &= "<td vAlign=top align=left><font size=2 face=Arial, Helvetica, sans-serif>" & oMensagem(i).NumeroItem & "</font></td>"
            sReportHTML &= "<td vAlign=top align=left><font size=2 face=Arial, Helvetica, sans-serif>" & oMensagem(i).CodigoProduto & "</font></td>"
            sReportHTML &= "<td vAlign=top align=left><font size=2 face=Arial, Helvetica, sans-serif>" & oMensagem(i).Descricao & "</font></td>"
            sReportHTML &= "<td vAlign=top align=left><font size=2 face=Arial, Helvetica, sans-serif>" & oMensagem(i).Status & "</font></td>"
            sReportHTML &= "</tr>"
            Application.DoEvents()
        Next i
    ...
    Private Function ReportErroNovas(ByVal sTitle As String, _
                               ByVal sSubTitle As String, _
                               ByVal oMensagemNovas() As MensagemErroNovas) As String
        ...
        'create the header row
        sReportHTML &= "<tr>"
        sReportHTML &= "<td width=10% vAlign=top align=left bgcolor=#99CCFF><strong><font size=2 face=Arial, Helvetica, sans-serif>CÓDIGO CONSULTORA</font></strong></td>"
        sReportHTML &= "<td width=10% vAlign=top align=left bgcolor=#99CCFF><strong><font size=2 face=Arial, Helvetica, sans-serif>SETOR</font></strong></td>"
        sReportHTML &= "<td width=30% vAlign=top align=left bgcolor=#99CCFF><strong><font size=2 face=Arial, Helvetica, sans-serif>NOME</font></strong></td>"
        sReportHTML &= "</tr>"

        frmMain.stbMain.Panels("pnlProgressBar").ProgressBarMaxValue = oMensagemNovas.Count
        frmMain.stbMain.Panels("pnlProgressBar").ProgressBarValue = 0
        frmMain.stbMain.Panels("pnlProgressBar").Visible = True

        For i As Integer = 0 To UBound(oMensagemNovas)

            frmMain.stbMain.Panels("pnlProgressBar").ProgressBarValue += 1

            sReportHTML &= "<tr>"
            sReportHTML &= "<td vAlign=top align=left><font size=2 face=Arial, Helvetica, sans-serif>" & oMensagemNovas(i).CodigoConsultora & "</font></td>"
            sReportHTML &= "<td vAlign=top align=left><font size=2 face=Arial, Helvetica, sans-serif>" & oMensagemNovas(i).Setor & "</font></td>"
            sReportHTML &= "<td vAlign=top align=left><font size=2 face=Arial, Helvetica, sans-serif>" & oMensagemNovas(i).RazaoSocial & "</font></td>"
            sReportHTML &= "</tr>"
            Application.DoEvents()
        Next i
    ...
    Private Sub EmailNFe(ByVal lCodigoEmissaoCapa As Long, _
                         ...
                         ByVal bBoleto As Boolean, _
                         ByVal bNFe As Boolean, _
                         ByVal bSMS As Boolean)
        ...
        Try
            ...
            If iStatus = CInt(StatusNotaFiscalEletronica.AutorizadaUso) Or _
               ...
               iStatus = CInt(StatusNotaFiscalEletronica.AguardandoTransmissao) Or _
               iStatus = CInt(StatusNotaFiscalEletronica.Cancelada) Then
                ...
                Dim sArquivoBoleto As String = ""
                Dim sArquivoBoleto2 As String = ""
                Dim sArquivoReport As String = ""
                Dim sMensagemErro As String = ""
                Dim sArquivoNFe As String = ""
                Dim sArquivoProtocoloNFe As String = ""
                Dim sArquivoCancelamentoNFe As String = ""
                ...
                If bNFe = True Then
                    If iStatus <> CInt(StatusNotaFiscalEletronica.AguardandoTransmissao) Then
                        Try
                            'Váriavel Relatório
                            sArquivoReport = goConfiguracaoNFe.sPastaImpressao & sNotaFiscal & ".pdf"
                        ...
                        Else
                            ...
                            End If
                        Catch ex As Exception
                            grdNotaFiscalLote.SetValue("mensagem", "Não foi possivel gerar o DANFE.")
                            sArquivoReport = ""
                        End Try
                    Else
                        grdNotaFiscalLote.SetValue("mensagem", "Favor fazer a transmissão da NFe antes de enviar o DANFE")
                        sArquivoReport = ""
                    End If
                End If

                If bBoleto = True Then
                    Try
                    ...
                    Dim sDiretorio As String = goConfiguracaoNFe.sPastaImpressao + "boleto\" + Format(Now, "yyMMdd_HHmmss") + "\"
                        ...
                        If lCodigoTituloCapa > 0 Then
                            ...
                            sArquivoBoleto2 = sPastaImpressao & "BOLETOS_" & "_" & Format(Now, "yyyyMMdd_hhmmss") & ".pdf"

                            If System.IO.Directory.Exists(sArquivoBoleto2) Then
                                Try
                                    System.IO.Directory.Delete(sArquivoBoleto2)
                                Catch ex As Exception
                        ...
                        End If
                    Catch ex As Exception
                        grdNotaFiscalLote.SetValue("mensagem", "Não foi possivel gerar o boleto.")
                        sArquivoBoleto2 = ""
                    End Try
                End If
                ...
                Dim sAnexos As String = goConfiguracaoNFe.sPastaImpressao & "\logo_magma.png;"

                If bBoleto = True Then
                    sAnexos = sAnexos & sArquivoBoleto2
                End If

                If bNFe = True Then
                    If bBoleto = True Then
                        sAnexos = sAnexos & ";" & sArquivoReport
                    Else
                        sAnexos = sAnexos & sArquivoReport
                    End If
                End If

                If sArquivoBoleto2 = "" And sArquivoReport = "" Then
                    sAnexos = ""
                End If

                'If bNFe = True Then
                '    If sAnexos = "" Then
                '        If sArquivoReport <> "" Then
                '            sAnexos = goConfiguracaoNFe.sPastaImpressao & "\logo_magma.png;" & sArquivoReport
                '        End If
                '    Else
                '        If sArquivoReport <> "" Then
                '            sAnexos = sAnexos & ";" & goConfiguracaoNFe.sPastaImpressao & "\logo_magma.png;" & sArquivoReport
                '        End If
                'End If
                ...
                Dim sAssunto As String = "Compra Tupperware - Distribuição Magma S" + grdNotaFiscalLote.GetValue("semana") + " - "

                If bNFe = True Then
                    sAssunto = sAssunto & "DANFE"
                End If

                If bBoleto = True Then
                    If sAssunto.Contains("DANFE") Then
                        sAssunto = sAssunto & " - BOLETO"
                    Else
                        sAssunto = sAssunto & "BOLETO"
                    End If
                End If
                ...
                Try
                    ...
                    End If
                Catch ex As Exception
                    grdNotaFiscalLote.SetValue("mensagem", "Não foi possivel fazer o envio do e-mail.")
                End Try
    ...
    Private Function LoadBodyEmail(ByVal sSemana As String, ByVal dDataEmissao As Date) As String
        Try
            ...
            If chkBoleto.Checked = True Then
                sBoleto = "<b> BOLETO </b>"
            Else
                sBoleto = ""
            End If

            If chkNFe.Checked = True Then
                If sBoleto = "" Then
                    sDANFE = "<b> DANFE </b>"
                Else
                    sDANFE = " e <b> DANFE </b>"
            ...
            Dim sBody As String = "<html>Prezado(a) consultor(a) Tupperware, segue em anexo" & IIf(sBoleto = "", "", sBoleto) & IIf(sDANFE = "", "", sDANFE) & " referente a sua compra da semana " & sSemana & " ."
            sBody += "<p class=MsoNormal><span style='font-size:11.0pt'></span></p>Conforme seu cadastro em nosso sistema, seu pedido deve ser pago antes da entrega, o pagamento deve ser realizado na quarta-feira."
            sBody += "<p class=MsoNormal><p class=MsoNormal>"
            sBody += "<b>O prazo para entrega do seu pedido inicia na quinta-feira e pode estender até sábado.</b><p class=MsoNormal>"
    ...
    Private Sub ExportarRelatorioConferencia()
        Dim sErro As String = ""
        Try

            Dim sCodigo As String = ""
            Dim oArquivo() As String
            Dim oCaminho() As String
            Dim j As Integer = 0
            Dim sDiretorio As String = "\\SRVFS\ACTi\NF-E\Pasta Impressão\"
            Dim i As Integer = 1
            ReDim oCaminho(i)
            ReDim oArquivo(1)
            oArquivo(0) = ""
            ...
            '--------------IMPRIMINDO A CONFERÊNCIA-----------------'
            'Váriaveis - Relatório
            Dim sReport As String = "rptRelatorioNFConferir.rpt"
            ...
            oArquivo(0) = sDiretorio + "CONFERENCIA_" + Format(Now, "ddMMyyyy_HHmmss") + ".pdf"

            'Exporta para pdf
            oRptDocument.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, oArquivo(0))
            ...
            'Váriaveis - Relatório
            sReport = "rptRelatorioNFRetiradaDocumentos.rpt"

            'Váriaveis Locais
            'Dim oRptDocument2 As New ReportDocument
            ...
            oArquivo(1) = sDiretorio + "DOCUMENTOS_" + Format(Now, "ddMMyyyy_HHmmss") + ".pdf"

            'Exporta para pdf
            oRptDocument.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, oArquivo(1))
            ...
            oPdfWriter = iTextSharp.text.pdf.PdfWriter.GetInstance(oPdfDoc, New FileStream(sDiretorio + "RELATORIO_" + Format(Now, "ddMMyyyy_HHmmss") + ".pdf", FileMode.Create))

            'Abre o document
            oPdfDoc.Open()

            AddPdf(oArquivo(0), oPdfDoc, oPdfWriter)
            AddPdf(oArquivo(1), oPdfDoc, oPdfWriter)
            ...
            Dim psi As New ProcessStartInfo
            psi.UseShellExecute = True
            psi.FileName = GerarPDF(sDiretorio + "RELATORIO_" + Format(Now, "ddMMyyyy_HHmmss") + ".pdf", oArquivo)
            Process.Start(psi)
            'Seta Cursor do Mouse
            Cursor.Current = Cursors.Default

            Try
                System.IO.File.Delete(oArquivo(0))
                System.IO.File.Delete(oArquivo(1))
            Catch ex As Exception
    ...
    Private Sub ImprimirDanfe(ByVal sCodigoEmissaoCapa As String)
        ...
        Try
            ...
            If VerificaDireito(Formulario.FaturamentoEmissaoNFe, gcPrint) = True Then
                ...
                Call ExportarRelatorioConferencia()

                'Seta Cursor do Mouse
                Cursor.Current = Cursors.Default

                'If MsgBox("Deseja imprimir o relatório de Conferência?", MsgBoxStyle.YesNo, "Relatório") = MsgBoxResult.Yes Then
                '    If sCodigoEmissaoCapa = "" Then
                '        frmMain.Informacao(Mensagem.ValidacaoSelecionar)
                '    Else
                ...
                '        Dim oReport2 As New usrReport
                '        oReport2.Dock = DockStyle.Fill

                '        frmMain.LoadPageReport("rptRelatorioNFConferir", "Conferência de Caixas", oReport2, frmMain.tabMain)
