...
Public Class clsUsrIndOrcamentoGanho
...
#Region "::: FUNCTION / SUB :::"
    ...
    Public Sub LoadGrafico(ByVal oChart As Chart, _
        ...
        Try
            ...
            'Executa Query
            oDataSet = ExecuteDataset(goDatabase.sConnection, CommandType.StoredProcedure, "sp_indicador_comercial_orcamento_ganho", oSqlParameter)
            ...
            '******************* CONFIGURA INDICADOR *******************
            oChart.Titles.Clear()
            oChart.Titles.Add("% ORÇAMENTOS GANHOS")
            ...
            'Adiciona Série
            oChart.Series.Add("ITENS ORÇADOS")
            oChart.Series.Add("ITENS GANHOS")
            oChart.Series.Add("ITENS GANHOS (%)")

            'Carrega Gráfico
            oChart.Series("ITENS ORÇADOS").Points.DataBind(oDataReader(0), "tipo", "quantidade_itens", "Tooltip=quantidade_itens")
            oChart.Series("ITENS GANHOS").Points.DataBind(oDataReader(1), "tipo", "quantidade_itens_ganhos", "Tooltip=quantidade_itens_ganhos")
            oChart.Series("ITENS GANHOS (%)").Points.DataBind(oDataReader(2), "tipo", "quantidade_itens_ganhos_porcentagem", "Tooltip=quantidade_itens_ganhos_porcentagem")
    ...
    Public Sub LoadGraficoW04(ByVal oChart As Chart)
        ...
        Try
            ...
            'Executa Query
            oDataSet = ExecuteDataset(goDatabase.sConnection, CommandType.StoredProcedure, "sp_acti_ind", oSqlParameter)
            ...
            'Formatando o eixo X de acordo com o valor escolhido
            oChart.ChartAreas(0).AxisX.Title = "STATUS"
            oChart.ChartAreas(0).AxisY.Title = "QUANTIDADE"

            'Adiciona Série
            oChart.Series.Add("TOTAL 2020")
            oChart.Series.Add("BUILT & PRINT")
            oChart.Series.Add("DESIGN & BUILT")

            'Carrega Gráfico
            oChart.Series("TOTAL 2020").Points.DataBind(oDataReader(0), "status", "total_2020", "Tooltip=descricao1")
            oChart.Series("BUILT & PRINT").Points.DataBind(oDataReader(1), "status", "built_print", "Tooltip=descricao2")
            oChart.Series("DESIGN & BUILT").Points.DataBind(oDataReader(2), "status", "design_built", "Tooltip=descricao3")
