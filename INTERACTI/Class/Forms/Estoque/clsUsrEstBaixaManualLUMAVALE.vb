...
Public Class clsUsrEstBaixaManualLUMAVALE
...
#Region "::: FUNCTION / SUB :::"
    ...
    Public Sub LoadGrid(ByVal oGrid As Janus.Windows.GridEX.GridEX, _
        ...
        Try
            ...
            oSqlParameter(1) = New SqlParameter
            oSqlParameter(1).ParameterName = "observacao"
            oSqlParameter(1).Direction = ParameterDirection.Input
            oSqlParameter(1).SqlDbType = SqlDbType.VarChar
            oSqlParameter(1).Value = sObservacao

            oSqlParameter(2) = New SqlParameter
            oSqlParameter(2).ParameterName = "data_inicio"
            oSqlParameter(2).Direction = ParameterDirection.Input
            oSqlParameter(2).SqlDbType = SqlDbType.Date
            oSqlParameter(2).Value = IIf(dDataInicio = "", DBNull.Value, dDataInicio)

            oSqlParameter(3) = New SqlParameter
            oSqlParameter(3).ParameterName = "data_termino"
            oSqlParameter(3).Direction = ParameterDirection.Input
            oSqlParameter(3).SqlDbType = SqlDbType.Date
            oSqlParameter(3).Value = IIf(dDataTermino = "", DBNull.Value, dDataTermino)


            'Executa Query
            oDataSet = ExecuteDataset(goDatabase.sConnection, CommandType.StoredProcedure, "sp_select_estoque_baixa", oSqlParameter)
